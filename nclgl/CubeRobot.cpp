...
void CubeRobot::Update(float dt) {
	...
	head->SetTransform(head->GetTransform() * Matrix4::Rotation(-1000.0f * dt, Vector3(0, 1, 0)));
	leftArm->SetTransform(leftArm->GetTransform() * Matrix4::Rotation(-1000.0f * dt, Vector3(1, 0, 0)));
	rightArm->SetTransform(rightArm->GetTransform() * Matrix4::Rotation(1000.0f * dt, Vector3(1, 0, 0)));
	SceneNode::Update(dt);
}
